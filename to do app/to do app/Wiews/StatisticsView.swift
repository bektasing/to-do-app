        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                HStack {
                    ...
                    Text(localizationManager.localized("statistics_analytics"))
                        .font(.largeTitle)
                        .fontWeight(.bold)
                    Spacer()
...
struct SummaryCardsView: View {
    ...
    var body: some View {
        HStack(spacing: 15) {
            ...
            StatCard(
                icon: "flame.fill",
                title: localizationManager.localized("current_streak"),
                value: "\(statsManager.streakData.currentStreak)",
                subtitle: localizationManager.localized("days_in_row"),
                color: .orange
            )
            ...
            StatCard(
                icon: "star.fill",
                title: localizationManager.localized("longest_streak"),
                value: "\(statsManager.streakData.longestStreak)",
                subtitle: localizationManager.localized("days"),
                color: .yellow
            )
            ...
            StatCard(
                ...
                title: localizationManager.localized("completion"),
                value: String(format: "%.0f%%", goalData.percentage),
                subtitle: "\(goalData.completed)/\(goalData.total) \(localizationManager.localized("tasks"))",
                color: .green
            )
            ...
            StatCard(
                ...
                title: localizationManager.localized("active_days"),
                value: "\(statsManager.streakData.totalDaysActive)",
                subtitle: localizationManager.localized("total"),
                color: .blue
            )
        }
        .padding(.horizontal)
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Image(systemName: "chart.bar.fill")
                    .foregroundColor(.blue)
                Text(localizationManager.localized("weekly_completion_chart"))
                    .font(.headline)
                Spacer()
            }
            
            if weeklyData.isEmpty || weeklyData.allSatisfy({ $0.completedTasks == 0 }) {
                Text(localizationManager.localized("no_data_yet"))
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                    .padding()
            } else {
                Chart(weeklyData) { stat in
                    BarMark(
                        x: .value(localizationManager.localized("day"), stat.shortDayName),
                        y: .value(localizationManager.localized("completed"), stat.completedTasks)
                    )
                    .foregroundStyle(
                        LinearGradient(
                            colors: [.green, .blue],
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Text(localizationManager.localized("monthly_productivity_chart"))
                    .font(.headline)
                Spacer()
            }
            ...
            if monthlyData.isEmpty || monthlyData.allSatisfy({ $0.completedTasks == 0 }) {
                ...
                Chart(monthlyData) { stat in
                    LineMark(
                        x: .value(localizationManager.localized("month"), stat.month),
                        y: .value(localizationManager.localized("completed"), stat.completedTasks)
                    )
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Text(localizationManager.localized("hourly_productivity"))
                    .font(.headline)
                Spacer()
                
                if mostProductiveHour.count > 0 {
                    VStack(alignment: .trailing, spacing: 2) {
                        Text(localizationManager.localized("most_productive_hour"))
                            .font(.caption2)
                            .foregroundColor(.secondary)
            ...
            if hourlyData.allSatisfy({ $0.tasksCompleted == 0 }) {
                ...
                Chart(hourlyData.filter { $0.tasksCompleted > 0 }) { stat in
                    BarMark(
                        x: .value(localizationManager.localized("hour"), stat.hourLabel),
                        y: .value(localizationManager.localized("task"), stat.tasksCompleted)
                    )
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Text(localizationManager.localized("most_productive_day"))
                    .font(.headline)
                Spacer()
            }
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(mostProductiveDay.count) \(localizationManager.localized("tasks_completed"))")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                }
